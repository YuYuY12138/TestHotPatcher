...
#endif
		TickDelegateHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateUObject(this, &UDownloadProxy::Tick));
...
void UDownloadProxy::OnDownloadProcess(FHttpRequestPtr RequestPtr, uint64 byteSent, uint64 byteReceive)
{
...
{	
	...
	HttpRequest->OnRequestProgress64().BindUObject(this, &UDownloadProxy::OnDownloadProcess);
...
static TArray<uint8>& GetResponseContentData(FHttpResponsePtr InHttpResponse)
{
	return const_cast<TArray<uint8>&>(InHttpResponse->GetContent());
}
