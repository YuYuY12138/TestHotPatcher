	...
	)
{
	...
	if (bUseZenLoader)
	{
		PackageWriter = new FZenStoreWriter(ResolvedProjectPath, ResolvedMetadataPath, TargetPlatform,
			MakeShareable((ICookArtifactReader*)nullptr));
...
bool UFlibHotPatcherCoreHelper::RunCmdlet(const FString& CmdletName,const FString& Params,int32& OutRetValue)
{
	...
	UClass* SPCTCmdletClass = FindFirstObject<UClass>(*CmdletNameStr, EFindFirstObjectOptions::None, ELogVerbosity::NoLogging);
...
#if !UE_VERSION_OLDER_THAN(5,4,0)
		// GetCookModificationDelegate was removed in UE 5.4+
#else
		PRAGMA_DISABLE_DEPRECATION_WARNINGS;
	...
	)
{
	...
	if (bStorageConcurrent)
	{
		...
		for (auto WorldIt = WorldsToPostSaveRoot.CreateConstIterator(); WorldIt; ++WorldIt)
		{
			...
			check(World);
			FObjectSaveContextData PostRootData;
			FObjectPostSaveRootContext PostRootContext(PostRootData);
