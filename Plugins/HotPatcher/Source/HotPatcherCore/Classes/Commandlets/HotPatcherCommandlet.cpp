...
int32 UHotPatcherCommandlet::Main(const FString& Params)
{
	...
	if (FFileHelper::LoadFileToString(JsonContent, *config_path))
	{
		...
		// Clean up PatcherProxy and force GC before engine exit to avoid UE 5.7 WorldCookPackageSplitter crash
		PatcherProxy->RemoveFromRoot();
		PatcherProxy = nullptr;
		CollectGarbage(GARBAGE_COLLECTION_KEEPFLAGS, true);
	}
	
#if WITH_UE5
	// Reset cook flag to prevent UWorldPartition::BeginDestroy from calling UnregisterCookPackageSubSplitterFactory
	// which asserts in UE 5.7 when the static RegisteredCookPackageSubSplitterFactories is already destroyed
