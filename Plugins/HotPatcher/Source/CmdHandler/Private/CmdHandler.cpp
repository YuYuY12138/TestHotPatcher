...
bool AddMultiCookerBackendToConfig(const FString& DDCAddr)
{
	...
	auto MultiCookerDDCBackendSection = const_cast<FConfigSection*>(EngineIniIns->FindSection(MultiCookerDDCBackendName));
	if (!MultiCookerDDCBackendSection)
	{
		MultiCookerDDCBackendSection = &EngineIniIns->Add(MultiCookerDDCBackendName, FConfigSection());
	}
